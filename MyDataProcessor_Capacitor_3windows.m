function [stc, stc1, stc2] = MyDataProcessor_Capacitor_3windows(data, unit, Export)
%{
data: 5*n 矩阵 (5 行多列)
    frequency
    Phase_deg
...
% 找到谐振频率 (假设 data 已经过滤波处理)
    X_s_abs = abs(data(5, :));
    Index = MyArrayProcessor_findLocalMinimum(X_s_abs);  % 找最小值
...
% 作图 1
    ...
    stc.layout = tiledlayout(1, 3, 'Parent', stc.fig, 'TileSpacing', 'loose', 'Padding', 'loose');
    ax = nexttile;
    stc1 = MyPlot_Capacitor_2models_ax(ax, f, C_cr*multiplier, C_clr*multiplier); 
    stc1.label.y.String = ['Capacitance $C$ (', unit, ')'];
    f_max = 0.75*f_0;
    xlim([100, f_max])
    C_min = min( min(C_cr(data(1, :)<f_max)), min(C_clr(data(1, :)<f_max)) );
    C_max = max( max(C_cr(data(1, :)<f_max)), max(C_clr(data(1, :)<f_max)) );
    ylim([0.98*C_min*multiplier, 1.01*C_max*multiplier])

% 作图 2
    stc2 = MyYYPlot_ImpedanceAndResistance_layout(stc.layout, data(1, :), abs(data(3, :)), data(2, :), abs(data(4, :)), abs(data(5, :)));
...
% 保存 pdf
    windowSize = [2048 512];
    MyFigure_ChangeSize(stc.fig, windowSize);
    if Export
