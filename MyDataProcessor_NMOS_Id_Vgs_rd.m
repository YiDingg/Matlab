function stc = MyDataProcessor_NMOS_Id_Vgs_rd(data)
...
% 作图
    ...
    for i = 1:len
        if i == 1
            stc.(['num', (num2str(i))]) = MyPlot(data(3, index_min(i):index_max(i)), data(2, index_min(i):index_max(i)));
            stc.(['num', (num2str(i))]).plot.Color = c{i};
            %stc.(['num', (num2str(i))]).leg.String(i) = cellstr(num2str(leg_str(i, :)));
        else
            stc.(['num', (num2str(i))]) = MyPlot_ax(stc.num1.axes, data(3, index_min(i):index_max(i)), data(2, index_min(i):index_max(i)));
