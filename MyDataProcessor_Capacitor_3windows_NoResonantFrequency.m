function [stc, stc1, stc2] = MyDataProcessor_Capacitor_3windows_NoResonantFrequency(data, unit, Export)
%{
data: 5*n 矩阵 (5 行多列)
    frequency
    Phase_deg
...
% 仅构建 C-Rs 模型
    L_s = 1/(C_1KHz*(2*pi*10e6)^2); % L_s 小于此值
    f = data(1, :);
    X_s =  data(5, :);
    C_cr = 1./(-2*pi*f.*X_s);
...
% 作图 1
    ...
    ax = nexttile;
    stc1 = MyPlot_Capacitor_ax(ax, f, C_cr*multiplier); 
